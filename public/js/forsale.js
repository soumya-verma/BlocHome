...
document
  .querySelector("#updateOwnLandsButtonID")
  .addEventListener("click", loadOwnedLands);
...
function buyProperty() {
  let isConfirm = confirm(
    `Initiating transaction with ${this.dataset.owner} for land id ${this.dataset.landid}, Sure you want to proceed?`
  );
  if (!isConfirm) return;
  console.log(isConfirm);
  realEstateContractHook.transferLand(
    ethWeb3.eth.defaultAccount,
    this.dataset.landid,
    (err, ack) => {
      console.log(ack);
      if (err) throw err;
...
function updateOwnListDisplay(land) {
  document.querySelector(".ownLandsPanel").innerHTML += `
    <div class="w3-third">
        <div class="w3-card-4 property">
            <table>
                <tr><td><span class="fa fa-info"></span>&nbsp; LandID</td><td>${
                  land[3]
                }</td></tr>
                <tr><td><span class="fa fa-user"></span>&nbsp; Owner</td><td title="${
                  land[2]
                }">${land[2].slice(0, 10)}...${land[2].slice(-10)}</td></tr>
                <tr><td><span class="fa fa-money"></span> Price</td><td>${
                  land[1]
                }</td></tr>
                <tr><td><span class="fa fa-map-marker"></span>&nbsp;&nbsp;Location</td><td>${
                  land[0]
                }</td></tr>
            </table>
            <div style="text-align:center;padding-top:20px;">
                <button data-landid="${land[3]}" data-owner="${
    land[2]
  }" data-price="${land[1]}" data-location="${
    land[0]
  }" class="w3-btn buy-property-button" style="background-color:#09f;color:#fff;">Buy</button>
            </div>
        </div>
    </div>
    `;
  ...
  document
    .querySelector(".buy-property-button")
    .addEventListener("click", buyProperty);
}
