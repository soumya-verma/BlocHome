...
if (typeof web3 !== "undefined") {
  ...
  realEstateContract = new ethWeb3.eth.contract(contractABI);
  realEstateContractHook = realEstateContract.at(contractAddress);
  realEstateContractAddEvent = realEstateContractHook.Add({}, "latest");
  realEstateContractTransferEvent = realEstateContractHook.Transfer(
    {},
    "latest"
  );
  updateStatus("\nRegistered for events from Contract");
  realEstateContractAddEvent.watch(function (error, result) {
    if (!error) {
      updateStatus(
        "\nAdd Event: Owner: " +
          result.args._owner +
  ...
  realEstateContractTransferEvent.watch(function (error, result) {
    if (!error) {
      updateStatus(
        "\nTransfer Event: From: " +
          result.args._from +
  ...
  var message = $(
    "<li><h5>Metamask Not available. Install and Try again</h5></li>"
  );
  $("#initialStatusID").append(message);
